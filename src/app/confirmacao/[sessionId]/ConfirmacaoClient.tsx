      ...
      <div className="relative z-10 max-w-lg mx-auto w-full px-4 py-10">
        ...
        >
          ...
          <h1 className="font-playfair font-bold text-3xl sm:text-4xl text-white mb-2">
            Pedido <span style={{ color: '#FFD700' }}>Confirmado!</span>
          </h1>
          ...
          {whatsapp && (
            <p className="text-white/40 text-sm mt-1">
              📲 Enviamos confirmação para o WhatsApp {maskWhatsapp(whatsapp)}
            </p>
          )}
        </motion.div>
        ...
        >
              ...
              <div className="flex-1">
                ...
                <div className="mt-3 space-y-1">
                  <div className="flex items-center gap-2 text-sm text-white/60">
                    <span>🪑</span>
                    <span>{setorNome} — {tipo === 'meia' ? 'Meia Entrada' : 'Inteira'}</span>
                  </div>
                  <div className="flex items-center gap-3">
                    <span className="text-white/50 text-sm">{quantidade}× ingresso(s)</span>
                    <span className="font-bold text-white text-lg">{formatPrice(valorTotal)}</span>
                  </div>
                </div>
              </div>

              <div className="rounded-xl overflow-hidden shrink-0 p-1" style={{ backgroundColor: 'white' }}>
          ...
          <div className="px-6 py-4">
            <div
              className="border-t"
              style={{ borderTopStyle: 'dashed', borderTopColor: 'rgba(255,255,255,0.15)' }}
            />
          </div>
          ...
          <div className="px-6 pb-6 flex flex-col items-center gap-3">
            ...
            <p className="text-white/20 text-xs font-mono">{codigoIngresso}</p>
          </div>
        </motion.div>
        ...
        >
          ...
          {espetaculo && (
            <button
              ...
              className="w-full py-3 rounded-xl font-semibold text-white text-sm transition-all hover:bg-white/10"
                    ...
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        ...
                        <p className="text-white/40 text-xs mt-0.5">
                          {d.toLocaleDateString('pt-BR', { day: '2-digit', month: 'short' })} • {ev.cidade}
                        </p>
                      </div>
